...
const serviceTiles = document.querySelectorAll("[data-service]");
...
const serviceModalCloseControls = document.querySelectorAll("[data-service-close]");
...
function serviceModalOpen() {
  return Boolean(serviceModal && !serviceModal.hidden);
}
...
function isEditableTarget(target) {
  ...
  if (target.closest("[data-service-modal]")) return true;
...
window.addEventListener("keydown", (event) => {
  if (event.key === "Escape" && serviceModalOpen()) {
    event.preventDefault();
    closeServiceModal();
    return;
  }
